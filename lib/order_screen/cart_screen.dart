...
import 'package:hammad_customer_0/components/add_address_button.dart';
...
class _CartScreenState extends State<CartScreen> {
                      ...
                      showSnackBar(context, LocaleKeys.no_items_selected
                        ...
                        if (address == null) {
                          ...
                          Order order = Order(
                              ...
                              "",
                              "","1");
                          DatabaseService(context, user.id).writeNewOrder(order);
                          DatabaseService(context, user.id).updateUserPoints("$points");
  ...
  StreamBuilder addressesStreamBuilder () {
    return  StreamBuilder(
        stream: DatabaseService(context, user.id).getAddressesStream(),
        builder: (context, snapshot) {
          addresses = [];
          if (snapshot.hasData) {
            addresses = snapshot.data as List<Address>;
            if (pAddress == null) {
...
class _ItemCardState extends State<ItemCard> {
  ...
  Widget build(BuildContext context) {
    return StreamBuilder(
        stream: GenDatabaseService(context).getProductStream(widget.cartItem.id),
        builder: (context, snapshot) {
          if (snapshot.hasData) {
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Row(
                          mainAxisAlignment: MainAxisAlignment.spaceBetween,
                          children: [
                            Text("${LocaleKeys.jod.tr()} ${(product.price *
          ...
          return StreamBuilder(
              stream: GenDatabaseService(context).getCouponStream(widget.cartItem.id),
              builder: (context, snapshot) {
                if (snapshot.hasData) {
                  Product product = snapshot.data as Product;
                  price = product.price;
